...
namespace mssql
{
	...
	*/
	...
	bool OdbcStatement::reserved_bit(const size_t row_count, const size_t column)
	{
	...
	bool OdbcStatement::reserved_int(const size_t row_count, const size_t column)
	{
		auto& bound_datum = _preparedStorage->atIndex(column);
		auto& ind = bound_datum->get_ind_vec();
		const auto storage = bound_datum->get_storage();
	...
	bool OdbcStatement::reserved_decimal(const size_t row_count, const size_t column)
	{
		auto& bound_datum = _preparedStorage->atIndex(column);
		auto& ind = bound_datum->get_ind_vec();
		const auto storage = bound_datum->get_storage();
