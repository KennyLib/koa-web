...
namespace mssql
{
	...
	*/
	...
	bool OdbcStatement::dispatch_prepared(const SQLSMALLINT t, const size_t rows_read, const size_t column)
	{
		bool res;
		switch (t)
		{
		...
		case SQL_SS_XML:
		case SQL_GUID:
			res = reserved_string(rows_read, column);
			break;
		...
		case SQL_SS_UDT:
			res = reserved_binary(rows_read, column);
			break;
	...
	bool OdbcStatement::try_read_columns(const size_t number_rows)
	{
		...
		else
		{
			...
			for (auto c = 0; c < column_count; ++c) {
				const auto& definition = _resultset->get_meta_data(c);
				res = dispatch_prepared(definition.dataType, row_count, c);
				if (!res) {
					break;
				}
			}
	...
	bool OdbcStatement::reserved_string(const size_t row_count, const size_t column) const
	{
		auto& bound_datum = _preparedStorage->atIndex(column);
		auto& ind = bound_datum->get_ind_vec();
		const auto size = sizeof(uint16_t);
		const auto storage = bound_datum->get_storage();
		auto value_len = storage->uint16vec_ptr->size();
		value_len /= size;		
		for (size_t row_id = 0; row_id < row_count; ++row_id) {
			auto offset = value_len * row_id;
			const auto value = make_shared<StringColumn>(column, storage->uint16vec_ptr, offset, ind[row_id]);
			_resultset->add_column(row_id, value);
		}
		return true;
	}

	bool OdbcStatement::reserved_binary(const size_t row_count, const size_t column) const
	{
		auto& bound_datum = _preparedStorage->atIndex(column);
		auto& ind = bound_datum->get_ind_vec();
		const auto storage = bound_datum->get_storage();
		const auto value_len = storage->charvec_ptr->size();	
		for (size_t row_id = 0; row_id < row_count; ++row_id) {
			const auto str_len_or_ind_ptr = ind[row_id];
			if (str_len_or_ind_ptr == SQL_NULL_DATA)
			{
			...
			auto offset = value_len * row_id;
			const auto value = make_shared<BinaryColumn>(column, storage, offset, ind[0]);
			_resultset->add_column(row_id, value);
		}
		return true;
	}
