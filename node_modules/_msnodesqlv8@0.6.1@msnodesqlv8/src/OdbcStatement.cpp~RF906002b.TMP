	...
	*/
	...
	bool OdbcStatement::try_read_columns(const size_t number_rows)
	{
		...
		else
		{
			...
			SQLSetStmtAttr(statement, SQL_ATTR_ROWS_FETCHED_PTR, &row_count, 0);
			SQLSetStmtAttr(statement, SQL_ATTR_ROW_STATUS_PTR, row_status, 0);
			SQLRETURN ret = SQLFetchScroll(statement, SQL_FETCH_NEXT, 0);
			if (!check_odbc_error(ret)) return false;
			const auto column_count = static_cast<int>(_resultset->get_column_count());
			for (auto c = 0; c < column_count; ++c) {
				const auto& definition = _resultset->get_meta_data(c);
	...
	bool OdbcStatement::reserved_string(const size_t row_count, const size_t column) const
	{
		...
		const auto storage = bound_datum->get_storage();
		for (size_t row_id = 0; row_id < row_count; ++row_id) {
			auto offset = (value_len +1 ) * row_id;
			const auto value = make_shared<StringColumn>(column, storage->uint16vec_ptr, offset, value_len);
	...
	bool OdbcStatement::reserved_binary(const size_t row_count, const size_t column) const
	{
		...
		for (size_t row_id = 0; row_id < row_count; ++row_id) {
			...
			const auto value = make_shared<BinaryColumn>(column, storage, offset, value_len);
			_resultset->add_column(row_id, value);
		}
		return true;
	}
