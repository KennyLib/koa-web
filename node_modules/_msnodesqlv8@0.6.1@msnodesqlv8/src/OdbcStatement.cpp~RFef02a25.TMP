...
namespace mssql
{
	...
	bool OdbcStatement::try_read_columns(const size_t number_rows)
	{
		...
		_try_read_columns_last_ms = (std::clock() - c.get_counter()) / static_cast<double>(CLOCKS_PER_SEC / 1000);
		_try_read_columns_total_ms += _try_read_columns_last_ms;
		return res;
	}
	...
	Handle<Value> OdbcStatement::get_column_values() const
	{
		...
		if (_resultset->EndOfRows())
		{
			...
			result->Set(fact.from_two_byte(L"total_reader_ms"), fact.new_int64(_try_read_columns_total_ms));
		}
		...
		result->Set(fact.from_two_byte(L"reader_ms"), fact.new_int64(_try_read_columns_last_ms));
		for (size_t row_id = 0; row_id < number_rows; ++row_id) {
	...
	*/
	...
	{
		_try_read_columns_last_ms = 0;
		_try_read_columns_total_ms = 0;
		auto& ps = *params;
